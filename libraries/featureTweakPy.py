...
def search_path(estimator, class_labels, aim_label):
    ...
    for leaf_node in leaf_nodes:
        ...
        while (parent_node != 0):
            if leaf_node == 0:
                break
            elif (np.where(children_left == child_node)[0].shape == (0, )):
                parent_left = -1
...
def esatisfactory_instance(x, epsilon, path_info, cat_feature_ids = []):
    """
    ...
    for i in range(len(path_info['feature'])):
        ...
        if feature_idx in cat_feature_ids:
            _epsilon = 1
        else:
            _epsilon = epsilon
        ...
        if inequality_symbol == 0:
            esatisfactory[feature_idx] = threshold_value - _epsilon
...
def feature_tweaking_ensemble_loop(ensemble_classifier, x, class_labels, aim_label, epsilon, cost_func, iterate=1):
    """
    ...
    delta_mini = 10**10  # initialize cost
    for estimator in ensemble_classifier:
        if (ensemble_classifier.predict(x.reshape(1, -1)) == estimator.predict(x.reshape(1, -1))
            ...
            for key in paths_info:
                ...
                for _e in [1.1, 1.3, 1.5, 1.7, 1.9]:
                    es_instance = esatisfactory_instance(x, _e * epsilon, path_info)
                    #  we need t generate multiple es_instances by using
...
def feature_tweaking_ensemble(ensemble_classifier, x, class_labels, aim_label, epsilon, cost_func, cat_feature_ids = []):
    """
    This function return the active feature tweaking vector.
    x: feature vector
    class_labels: list containing the all class labels
    ...
    """ initialize """
    x_out = copy.deepcopy(x)  # initialize output
    delta_mini = 10**6  # initialize cost
    for estimator in ensemble_classifier:
        if (ensemble_classifier.predict(x.reshape(1, -1)) == estimator.predict(x.reshape(1, -1))
            and estimator.predict(x.reshape(1, -1) != aim_label)):
            paths_info = search_path(estimator, class_labels, aim_label)
            for key in paths_info:
                """ generate epsilon-satisfactory instance """
                path_info = paths_info[key]
                es_instance = esatisfactory_instance(x, epsilon, path_info, cat_feature_ids)
                if ensemble_classifier.predict(es_instance.reshape(1, -1)) == aim_label:
                    if cost_func(x, es_instance) < delta_mini:
                        x_out = es_instance
                        delta_mini = cost_func(x, es_instance)
...
def feature_tweaking_estimator(ensemble_classifier, x, class_labels, aim_label, epsilon, cost_func):
    """
    This function return the active feature tweaking vector.
    x: feature vector
    class_labels: list containing the all class labels
